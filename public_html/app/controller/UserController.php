...
class UserController implements Editable, Deletable {
    ...
    public function sendNotificationsToUnactiveUsers() {
        ...
        foreach ($notActiveUsers as $notActiveUser) {
            if (strtotime($notActiveUser->last_day) < strtotime($today) - strtotime('-' . MAX_DAYS_NOT_ACTIVE) &&
                (strtotime($notActiveUser->last_time_sent_email) < strtotime($today) - strtotime('-' . MAX_DAYS_NOT_ACTIVE) ||
